...
CREATE TABLE `temu_order` (
  ...
  `unit_price` decimal(10,6) DEFAULT NULL COMMENT '单位价格',
  `total_price` decimal(10,2) DEFAULT NULL COMMENT '总价',
  `price_rule` longtext COMMENT '记录价格规则',
  `default_price` decimal(10,2) DEFAULT NULL COMMENT '默认价格',
  `goods_sn` text COMMENT '商品条码',
  `compliance_url` text COMMENT '合规单URL',
  `remark` text COMMENT '备注',
  `original_quantity` int(11) DEFAULT NULL COMMENT 'temu官方后台显示的数量',
  `compliance_image_url` text COMMENT '合规单图片URL',
  `compliance_goods_merged_url` text COMMENT '合规单和商品条码PDF合并URL',
  `is_complete_draw_task` bit(1) DEFAULT b'0' COMMENT '是否完成作图 任务',
  `is_complete_producer_task` bit(1) DEFAULT b'0' COMMENT '是否完成生产 任务',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `idx_sku` (`sku`) USING BTREE COMMENT 'SKU查询索引',
  KEY `idx_shop_id` (`shop_id`) USING BTREE COMMENT '店铺ID查询索引',
  KEY `custom_sku` (`custom_sku`) USING BTREE COMMENT '定制SKU查询索引',
  KEY `skc` (`skc`) USING BTREE COMMENT 'SKC查询索引',
  KEY `idx_category_id` (`category_id`) USING BTREE COMMENT '类目ID查询索引',
  KEY `idx_order_no，id` (`order_no`,`id`) USING BTREE COMMENT '订单编号，id复合索引\r\n',
  KEY `idx_order_status` (`order_status`,`order_no`) USING BTREE COMMENT '订单状态索引',
  KEY `idx_create_time` (`create_time`)
) ENGINE=InnoDB AUTO_INCREMENT=13665 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='订单表';
